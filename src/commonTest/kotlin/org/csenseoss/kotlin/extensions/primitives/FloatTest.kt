...
class FloatTest {
    ...
    fun floatWithoutDecimalPart() {
        (-1).toFloat().withoutDecimalPart().assert(expected = -1.0f, delta = 0.001f)
        0.toFloat().withoutDecimalPart().assert(expected = 0.0f, delta = 0.001f)
        1.toFloat().withoutDecimalPart().assert(expected = 1.0f, delta = 0.001f)
        20.5f.withoutDecimalPart().assert(expected = 20.0f, delta = 0.001f)
        20.9f.withoutDecimalPart().assert(expected = 20.0f, delta = 0.001f)
        20.0001f.withoutDecimalPart()
            .assert(expected = 20.0f, delta = 0.0001f, message = "should be more precise than not doing it")
    }

    @Test
    fun floatWithDecimalPart() {
        (-1).toFloat().withDecimalPart(0.0f).assert(expected = -1.0f, delta = 0.0001f)
        0.toFloat().withDecimalPart(0.0f).assert(expected = 0.0f, delta = 0.0001f)
        1.toFloat().withDecimalPart(0.0f).assert(expected = 1.0f, delta = 0.0001f)
        1.5f.withDecimalPart(0.0f).assert(expected = 1.0f, delta = 0.0001f)
        1.6f.withDecimalPart(0.1f).assert(expected = 1.1f, delta = 0.0001f)
        90.0579f.withDecimalPart(0.01234f).assert(expected = 90.01234f, delta = 0.00001f)
    }

    @Test
    fun floatDecimalPart() {
        (-1).toFloat().decimalPart().assert(expected = 0.0f, delta = 0.0001f)
        0.toFloat().decimalPart().assert(expected = 0.0f, delta = 0.0001f)
        1.toFloat().decimalPart().assert(expected = 0.0f, delta = 0.0001f)
        100.5f.decimalPart().assert(expected = 0.5f, delta = 0.0001f)
        100.99f.decimalPart().assert(expected = 0.99f, delta = 0.0001f)
        100.0005f.decimalPart().assert(expected = 0.0005f, delta = 0.0001f)
    }
